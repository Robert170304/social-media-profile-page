...
function CreateAcc() {
    ...
    function gettingPathonChange(e) {
        setInputVals((oldVals) => {
            return {...oldVals, 
                profileImgSrc: e.target.files[0]
            }
        })
    }
    ...
    function saveUserData(e) {
        ...
        const {firstName, lastName, userName, confirmPassword, password} = inputVals
        ...
        if(firstName === ""){
            ...
            startTransition(() => {
                setInputVals((oldVals) => {
                    return {...oldVals, firstName: '', lastName: '', userName:'', confirmPassword: '', password:'', bio: '', location:'', profileImgSrc: ''}
                })
            })
  ...
  return (
    <motion.div 
    animate={{x: 0}} initial={{x: 1000}} transition={{type: 'spring',stiffness: 260,damping: 20}}
     className='flex justify-center flex-col gap-1 p-5 m-2 max-w-xl w-full'>
        <header className='flex justify-between items-center'>
            ...
            {!isPending ? <button type='submit' className='m-2 text-blue-500' onClick={saveUserData} >
        ...
        <div className="profile-img flex flex-col gap-y-2 items-center justify-center text-slate-200">
            ...
            <input type="file" id="ImgInput" name='profileImgSrc' onChange={gettingPathonChange}/>
            </> 
           : <img src={profileImgURL} alt="profile" className='profileImg w-1/4 h-40 rounded-full'/>}
        </div>
